...
def read_lightfield(data_folder):
    ...
    light_field = np.zeros((params["num_cams_x"], params["num_cams_y"], params["height"], params["width"], 3),
                           dtype=np.uint8)
...
def read_parameters(data_folder):
    ...
    with open(os.path.join(data_folder, "parameters.cfg"), "r") as f:
        # parser = ConfigParser.ConfigParser()  # python 2
        parser = configparser.ConfigParser()  # python 3

        # parser.readfp(f)  # python 2
        parser.read_file(f)  # python 3
...
def read_disparity(data_folder, highres=False):
    ...
    try:
        data = read_pfm(fpath)
        # print(f'see here ------> data shape: {data.shape}')
    except IOError:
...
def _get_next_line(f):
    next_line = f.readline().rstrip()  # ------------> new_line: b'Pf'
    next_line = str(next_line, encoding="utf-8")
    # ignore comments
